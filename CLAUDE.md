...
- **[#6] Handwarmer detection** — Port `wasHandwarmers()` scoring algorithm from [Melee-Ghost-Streamer](https://github.com/Sheepolution/Melee-Ghost-Streamer). Weighted score across: total damage < 50 (+1), LRAS end (+1), both players still had stocks (+2), kill count ≤ 1 (+1), duration < 45s (+1). If score ≥ 2, emit `game-end` with `{ winner: null, handwarmer: true }` and skip score increment. Uses `game.getStats().overall` and `game.getGameEnd()` — available at end of folder-mode `.slp` file.

- **[#7] Combo detection + auto replay queue** — After each game, scan `game.getStats().conversions` for highlight combos (≥ 4 moves, ≥ 30% damage, `didKill === true`). Emit `slippi_highlight` via Socket.io with frame range. Use OBS replay buffer + WebSocket API to save clips to a watched folder. OBS Python script (`auto_replays.py` pattern) polls the folder and queues clips into a VLC media source that plays between sets. Reference: `scripts/auto_replays.py` in Melee-Ghost-Streamer.

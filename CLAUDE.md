...
- **Each cam plate is open on the side facing the clip**, so the clip frame's outer edge closes the cam window and the two read as one bay rather than two windows separated by a rail. Three things move together and all three are load-bearing: the border width goes to `0`, the **element width drops by exactly one `--pad`** (zeroing the border alone stretches the hole a pad wider and misreports it in `?guides=1`), and that side's corners square off. Both hairlines drop to three sides via `--edge-inner` / `--edge-outer` — a four-sided `box-shadow` keeps drawing a gold line exactly where the removed rail was, which defeats the whole change. The `.link` bars take the same two variables: gold outward, dark on the face turned into the bay.

...
- **Geometry is nine `:root` variables**, so retuning is four numbers per window rather than a hunt through rules. Defaults match the replay scene's OBS transforms — clip `480,140 960×800`; cams `40,288` and `1480,288`, each `400×504`. Both cam centres and the clip centre sit at y=540; keep that when retuning. The cams are inset 40px from the canvas edges (they used to be flush) so the frames have room to cast a shadow.
- **`?guides=1`** outlines each frame's transparent hole and labels it with its *measured* rect — `getBoundingClientRect()` minus the border width, not a read-back of the CSS variables, so a bad `calc()` shows up there instead of on stream. That is the alignment tool; check it against OBS before a bracket.
- **`?animate=false`** freezes the title orbs and the sheen sweep, same convention as the side panel.
- **The scrim is four edge bands, not a full-canvas wash.** A browser source paints over every OBS source beneath it regardless of z-index, so a full-canvas scrim would tint the footage. Each band stops at the nearest window edge and fades inward; `--scrim-strength` scales all four, `0` turns them off.

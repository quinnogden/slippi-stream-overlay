...
- **The cams link to the clip frame at the border only — the gap stays open.** Four `.link` bars carry each cam plate's top and bottom border across to the clip plate, `--cam-pad` thick to match the border they continue, sized from `--join-l` / `--join-r` (derived from the geometry, `max(0px, …)` so cams pushed past the clip collapse the link instead of inverting it). The middle of each gap is left transparent; filling it walls off the blurred background and reads as a slab. **One bar per corner, not one tall element with a transparent middle** — a box-shadow traces the border box rather than the painted bands, so the hairlines and drop shadow would run down the open middle.
- **Each cam plate is open on the side facing the clip**, so the clip frame's outer edge closes the cam window and the two read as one bay rather than two windows separated by a rail. Three things move together and all three are load-bearing: the border width goes to `0`, the **element width drops by exactly one `--pad`** (zeroing the border alone stretches the hole a pad wider and misreports it in `?guides=1`), and that side's corners square off. **All three shadow layers drop to three sides** via `--edge-inner` / `--edge-outer` / `--depth` — left whole, the hairlines draw a gold line exactly where the removed rail was and the depth adds a soft vertical smudge beside it, which defeats the whole change. `--depth` becomes three directional casts where **blur/2 equals the spread**, the trick that makes reach on the two perpendicular sides exactly zero, so a shadow travels only the way it is offset. The `.link` bars use the same two variables and nothing is drawn or cast on the face turned into the bay, so the bay stays empty background.

...
- **The cams link to the clip frame at the border only — the gap stays open.** Four `.link` bars carry each cam plate's top and bottom border across to the clip plate, `--cam-pad` thick to match the border they continue, sized from `--join-l` / `--join-r` (derived from the geometry, `max(0px, …)` so cams pushed past the clip collapse the link instead of inverting it). The middle of each gap is left transparent; filling it walls off the blurred background and reads as a slab. **One bar per corner, not one tall element with a transparent middle** — a box-shadow traces the border box rather than the painted bands, so the hairline and drop shadow would run down the open middle. The cams' inner corners square off, since a bar butting into the original radius leaves a crescent of background at the junction.
